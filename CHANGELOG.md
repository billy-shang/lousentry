...
- 内置 Web 控制台，账号 / 监控 / 推送配置写入 SQLite
- 推送渠道只保留钉钉、飞书、企业微信；移除 Telegram、Slack、Bark、蓝信、Server酱、PushPlus、WPUSH、通用 Webhook
- 移除 `admin.json` / `--data-dir` 一次性迁移逻辑
- 项目更名为 **漏哨 / LouSentry**，二进制改为 `lousentry`

## v2.6.0 (2025-06-24)

## 新增

- 增加 `--no-sleep/-ns` 或者环境变量 `NO_SLEEP` 用于禁用夜晚休眠，用于全天24小时运行!
- 增加 `linux/arm` 环境的自动化编译，适用于路由器、nas 等设备运行

## 修复

- 修复启明星辰数据源 `venustech` 描述信息内包含 `上一页、下一页` 的问题

## v2.5.1 (2025-04-27)

### 修复
- 修复 seebug 爬取报错的问题  #136
